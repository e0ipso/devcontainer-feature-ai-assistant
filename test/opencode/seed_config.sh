#!/bin/bash
# Tests configuration seeding for the opencode feature.
set -e
...
TARGET="${USER_HOME}/.config/opencode/opencode.json"
SEED_DIR="${USER_HOME}/.cred-seed/opencode"

check "baked default present" test -f "${FEATURE_DIR}/opencode.json"
...
rm -f "${TARGET}"
mkdir -p "${SEED_DIR}"
echo '{"from":"host"}' > "${SEED_DIR}/opencode.json"
"${FEATURE_DIR}/seed-config.sh"
